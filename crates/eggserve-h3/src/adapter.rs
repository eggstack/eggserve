//! Experimental HTTP/3 server adapter (Plan 220 authority).
//!
...
//! Sibling modules (`endpoint`/`request`/`response`/`tunnel`) own protocol
//! mechanics; this module owns accept/connection/request dispatch over the
//! shared canonical kernel (no H3-specific service semantics).

use crate::{h3, h3_quinn, quinn};
...
/// Experimental H3 accept loop over a bound QUIC endpoint.
///
/// Narrow adapter entry: Quinn/H3 types stay crate-internal/doc-hidden;
/// all policy comes from the canonical server config plus H3-owned
/// transport server_config. Admission semaphores and `OpsContext` are passed
/// explicitly so the compatibility facade shares its pools with TCP
/// (no second budget). H3 remains experimental with Plan 192-195 blockers.
#[allow(clippy::too_many_arguments)]
pub async fn accept_loop<S: Service>(
    endpoint: h3_quinn::Endpoint,
    local_addr: std::net::SocketAddr,
    server_config: Arc<RuntimeConfig>,
    h3_config: Arc<Http3Config>,
    ops: OpsContext,
    file_sem: Arc<Semaphore>,
    service_sem: Arc<Semaphore>,
    tunnel_sem: Arc<Semaphore>,
    ...
    ops.emit(eggserve_server::ops::Event::new(
        eggserve_server::ops::Severity::Info,
        eggserve_server::ops::EventKind::ListenerReady,
        "HTTP/3 QUIC endpoint started",
    ));

    let mut tasks = tokio::task::JoinSet::new();
    let pending_handshakes = Arc::new(Semaphore::new(h3_config.max_pending_handshakes));
    loop {
        tokio::select! {
            incoming = endpoint.accept() => {
                ...
                let service_sem = service_sem.clone();
                let tunnel_sem = tunnel_sem.clone();
                let connection_shutdown_rx = shutdown_rx.resubscribe();
                tasks.spawn(async move {
                        let _connection_permit = connection_permit;
                        let _active_connection = crate::endpoint::ActiveConnectionGuard { ops: ops.clone() };
                    let pending_permit = pending_permit;
                    let remote_addr = incoming.remote_address();
                    let connecting = if h3_config.stateless_retry && incoming.may_retry() {
                        ...
                        match incoming.accept() {
                            ...
                            Err(error) => {
                                ops.emit(eggserve_server::ops::Event::new(
                                    eggserve_server::ops::Severity::Debug,
                                    eggserve_server::ops::EventKind::TlsHandshakeFailure,
                    ...
                    let connection = match tokio::time::timeout(
                        server_config.tls_handshake_timeout,
                        connecting,
                    ).await {
                        ...
                        Err(_) => {
                            ops.emit(eggserve_server::ops::Event::new(
                                ...
                                eggserve_server::ops::EventKind::TlsHandshakeTimeout,
    ...
    let deadline = tokio::time::Instant::now() + server_config.graceful_shutdown_timeout;
    loop {
...
async fn serve_connection<S: Service>(
    ...
    builder.max_field_section_size(h3_config.max_field_section_size);
        ...
        Err(error) => {
            ops.emit(
                eggserve_server::ops::Event::new(
                    eggserve_server::ops::Severity::Debug,
                    eggserve_server::ops::EventKind::ConnectionRejected,
    ...
    loop {
        tokio::select! {
            _ = shutdown_rx.recv(), if !draining => {
                ...
                draining = true;
                drain_deadline = Some(tokio::time::Instant::now() + server_config.graceful_shutdown_timeout);
            }
            _ = async {
            ...
            closed = connection_closed.closed() => {
                let reason = crate::endpoint::h3_connection_close_reason(&closed);
                requests_registry.cancel_all(reason, conn_id, &ops);
                break;
            }
            accepted = h3_connection.accept() => {
                let resolver = match accepted {
                    ...
                    Err(_) => {
                        requests_registry.cancel_all(
                            crate::endpoint::h3_connection_close_reason(
                                &connection_closed
                                    .close_reason()
                ...
                if !draining && server_config.max_requests_per_connection.is_some_and(|max| accepted_requests >= max) {
                    let _ = h3_connection.shutdown(0).await;
                    draining = true;
                    drain_deadline = Some(tokio::time::Instant::now() + server_config.graceful_shutdown_timeout);
                    ops.emit(
                        eggserve_server::ops::Event::new(
                            eggserve_server::ops::Severity::Debug,
                            eggserve_server::ops::EventKind::MaxRequestsClose,
    ...
    let deadline = drain_deadline
        .unwrap_or_else(|| tokio::time::Instant::now() + server_config.graceful_shutdown_timeout);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        if remaining.is_zero() {
            requests_registry.cancel_all(RequestCancellationReason::ServerShutdown, conn_id, &ops);
            requests.abort_all();
            return;
        }
    ...
    C::RecvStream: Send + 'static,
{
    ...
    let head = match crate::request::convert_request_head(&request, &server_config, conn_id, &ops) {
        Ok(head) => head,
        Err(error) => {
            let response = crate::response::runtime_error_response(
                error.status_code().as_u16(),
                is_head,
                &server_config,
            );
            if !crate::response::send_response_or_cancel(
                &mut send_stream,
                response,
                &server_config,
                &h3_config,
            ...
            .await
            {
                ...
                // `crate::response::send_response_or_cancel` on failure; still abort receive so
    ...
    let declared_length = match crate::request::declared_content_length(&request) {
        Ok(length) => length,
        Err(error) => {
            let _ = crate::response::send_response_or_cancel(
                &mut send_stream,
                crate::response::runtime_error_response(
                    error.status_code().as_u16(),
                    is_head,
                    &server_config,
                ),
    ...
    let policy = eggserve_server::connection::select_body_policy(
    ...
    if let Some(limit) = policy.max_bytes() {
        if declared_length.is_some_and(|length| length > limit) {
            let response = crate::response::runtime_error_response(413, is_head, &server_config);
            let _ = crate::response::send_response_or_cancel(
                &mut send_stream,
                response,
                &server_config,
    ...
    let request_body = if policy.is_reject() {
        ...
        let presence =
            tokio::time::timeout(server_config.body_read_timeout, recv_stream.recv_data()).await;
        match presence {
            Ok(Ok(Some(_data))) => {
                let response =
                    crate::response::runtime_error_response(413, is_head, &server_config);
                let _ = crate::response::send_response_or_cancel(
                    &mut send_stream,
                    response,
                    &server_config,
            ...
            Ok(Ok(None)) => {
                ...
                eggserve_primitives::request_body::RequestBody::from_incoming_with_shared(
                    stream::empty(),
                    Some(0),
                    0,
                    shared.clone(),
            ...
            Ok(Err(_error)) => {
                ...
                let _ = crate::response::send_response_or_cancel(
                    &mut send_stream,
                    crate::response::runtime_error_response(500, is_head, &server_config),
                    &server_config,
                    &h3_config,
                    is_head,
                    &file_sem,
            ...
            Err(_) => {
                ...
                let _ = crate::response::send_response_or_cancel(
                    &mut send_stream,
                    crate::response::runtime_error_response(408, is_head, &server_config),
                    &server_config,
                    &h3_config,
                    is_head,
                    &file_sem,
        ...
        let wire_slot = eggserve_primitives::request_body::new_wire_slot();
                    ...
                    tokio::select! {
                        ...
                        data = stream.recv_data() => match data {
                            ...
                            Ok(None) => {
                                ...
                                match probe {
                                    Ok(Ok(Some(map))) => {
                                        match crate::request::h3_trailers_to_block(&map) {
                                            Ok(block) => {
        ...
        let request_body =
            eggserve_primitives::request_body::RequestBody::from_incoming_with_shared_and_wire_slot(
                ...
                policy
                    .max_bytes()
                    .unwrap_or(server_config.max_request_body_bytes),
                shared.clone(),
                wire_slot,
            );
        crate::response::spawn_body_timeout_watchdog(
            shared.clone(),
            body_cancel.0,
            tokio::time::Instant::now() + server_config.body_read_timeout,
            conn_id,
            ops.clone(),
        );
        request_body
    ...
    let request_body = match policy {
        eggserve_primitives::request_body_policy::RequestBodyPolicy::Buffer { max_bytes } => {
            match tokio::time::timeout(
                server_config.body_read_timeout,
            ...
            .await
            {
                Ok(Ok((bytes, trailers))) => {
                    ...
                    let mut body =
                        eggserve_primitives::request_body::RequestBody::from_bytes_with_shared(
                            bytes,
                            max_bytes,
                            shared.clone(),
                        );
    ...
    let permit = match service_sem.clone().try_acquire_owned() {
        Ok(permit) => permit,
        Err(_) => {
            let _ = crate::response::send_response_or_cancel(
                &mut send_stream,
                crate::response::runtime_error_response(503, is_head, &server_config),
                &server_config,
                &h3_config,
                is_head,
                &file_sem,
    ...
    let result =
        crate::request::invoke_service(service, request, permit, &server_config, &ops).await;
    let response = match result {
        Ok(response) => response,
        Err(error) => crate::response::runtime_error_response(
            error.status_code().as_u16(),
            is_head,
            &server_config,
        ),
...
async fn handle_h3_connect<S, C>(
    ...
    C::RecvStream: Send + 'static,
{
    use eggserve_primitives::tunnel::{classify_extended_protocol, TunnelKind, TunnelRequest};
    use eggserve_server::tunnel::TunnelIo;

    let ops = ops.clone();
    // Bodies never cross the transition (Track H).
    match crate::request::declared_content_length(&request) {
        Ok(Some(len)) if len > 0 => {
            let _ = crate::response::send_response_or_cancel(
                &mut send_stream,
                crate::response::runtime_error_response(413, false, &server_config),
                &server_config,
                &h3_config,
                false,
                &file_sem,
    ...
    let (kind, protocol) = match protocol_raw {
        Some(raw) => match classify_extended_protocol(Some(&raw)) {
            ...
            None => {
                ...
                let _ = crate::response::send_response_or_cancel(
                    &mut send_stream,
                    crate::response::runtime_error_response(400, false, &server_config),
                    &server_config,
                    &h3_config,
                    false,
                    &file_sem,
    ...
    let request_body = eggserve_primitives::request_body::RequestBody::from_incoming_with_shared(
    ...
    let request_for_service =
        eggserve_primitives::request::Request::new_with_context(head, request_body, ctx);
    ...
    let permit = match service_sem.clone().try_acquire_owned() {
        ...
        Err(_) => {
            ...
            let _ = crate::response::send_response_or_cancel(
                &mut send_stream,
                crate::response::runtime_error_response(503, false, &server_config),
                &server_config,
                &h3_config,
                false,
                &file_sem,
    ...
    let result = tokio::time::timeout(
        server_config.handler_timeout,
        eggserve_server::connection::contain_service_panic(
            service.call_with_tunnel(request_for_service, Some(server_capability)),
        ),
    )
    .await
    .unwrap_or_else(|_| {
        Err(eggserve_server::service::ServiceError::timeout(
            "handler timed out",
        ))
    });
    if let Some(s) = interim.as_ref() {
    ...
    let tunnel_permit = match tunnel_sem.clone().try_acquire_owned() {
        Ok(p) => p,
        Err(_) => {
            ops.counters()
                .tunnels_rejected
            ...
            ops.emit(
                eggserve_server::ops::Event::new(
                    eggserve_server::ops::Severity::Warn,
                    eggserve_server::ops::EventKind::TunnelRejected,
    ...
    // Re-attach empty body for `crate::response::send_canonical_response`-style conversion?
    // Instead, send headers directly (no body, no trailers, no finish).
    let send_result = crate::tunnel::send_h3_tunnel_handshake(
        &mut send_stream,
        handshake,
        &server_config,
        &h3_config,
    ...
    if send_result.is_err() {
        ...
        ops.emit(
            eggserve_server::ops::Event::new(
                eggserve_server::ops::Severity::Debug,
                eggserve_server::ops::EventKind::TunnelUpgradeFailed,
                "H3 tunnel handshake failed",
            )
            .connection_id(conn_id),
        );
        eggserve_server::connection::cancel_shared_with_observability(
            &shared,
    ...
    ops.emit(
        eggserve_server::ops::Event::new(
            eggserve_server::ops::Severity::Info,
            eggserve_server::ops::EventKind::TunnelAccepted,
            format!("tunnel accepted: {}", acceptance.kind),
        )
        .connection_id(conn_id),
    );
    let _active_guard = crate::tunnel::H3ActiveTunnelGuard { ops: ops.clone() };
    ...
    if let Ok(mut send_stream) = send_res {
        let _ =
            tokio::time::timeout(server_config.response_write_timeout, send_stream.finish()).await;
    }
    ...
    ops.emit(
        eggserve_server::ops::Event::new(
            eggserve_server::ops::Severity::Debug,
            eggserve_server::ops::EventKind::TunnelClosed,
            format!("tunnel closed: {}", crate::tunnel::kind_string(kind)),
        )
        .connection_id(conn_id),
    );
}

/// Apply runtime-owned `Alt-Svc` advertisement for H3 (Plan 220).
///
/// Generic privacy (`Server`/`Date`/denylist) already ran via
/// `finalize_canonical_response`; this H3-owned step adds
/// `h3=":<port>"; ma=86400` using the resolved same-port TCP bind when
/// `advertise_alt_svc` is set. Suppressed when the denylist contains
/// `alt-svc` or when H3 is disabled. No second privacy implementation:
/// denylist/`Server`/`Date` stay in the server kernel.
pub fn apply_alt_svc(
    mut response: eggserve_primitives::canonical::Response,
    server_config: &eggserve_server::config::RuntimeConfig,
    h3_config: &crate::config::Http3Config,
    ...
    }
    {
        ...
        if h3_config.advertise_alt_svc
            && !server_config
                .response_policy
                ...
                .any(|name| name.eq_ignore_ascii_case("alt-svc"))
        {
            let _ = headers.push_str(
                "alt-svc",
                format!("h3=\":{}\"; ma=86400", server_config.bind.port()),
            );
        }
    }
    response
...
mod tests {
    ...
    fn request_metadata_maps_to_canonical_http3_fields() {
        let request = hyper::Request::builder()
            ...
            .body(())
            .unwrap();
        let head = crate::request::convert_request_head(
            &request,
            &config(),
            7,
            eggserve_server::ops::OpsContext::global(),
        )
        .unwrap();
    ...
    fn request_metadata_rejects_connection_headers_and_invalid_te() {
        for (name, value) in [("connection", "close"), ("te", "chunked")] {
            let request = hyper::Request::builder()
                ...
                .body(())
                .unwrap();
            let error = crate::request::convert_request_head(
                &request,
                &config(),
                1,
                eggserve_server::ops::OpsContext::global(),
    ...
    fn declared_content_length_is_strict_and_duplicate_safe() {
        let request = hyper::Request::builder()
            ...
            .unwrap();
        assert_eq!(
            crate::request::declared_content_length(&request).unwrap(),
            Some(12)
        );
        ...
        let request = hyper::Request::builder()
            ...
            .unwrap();
        assert_eq!(
            crate::request::declared_content_length(&request)
                .unwrap_err()
                .status_code()
                .as_u16(),
            400
    ...
    fn http3_config_rejects_missing_unidirectional_stream_headroom() {
        let cfg = crate::config::Http3Config {
            enabled: true,
    ...
    async fn body_timeout_marks_h3_body_failed_and_wakes_stream() {
        ...
        crate::response::spawn_body_timeout_watchdog(
            ...
            1,
            eggserve_server::ops::OpsContext::global().clone(),
        );
    ...
    fn connection_close_reasons_use_transport_neutral_taxonomy() {
        assert_eq!(
            crate::endpoint::h3_connection_close_reason(&quinn::ConnectionError::TimedOut),
            RequestCancellationReason::ConnectionTimeout
        );
        assert_eq!(
            crate::endpoint::h3_connection_close_reason(&quinn::ConnectionError::Reset),
            RequestCancellationReason::PeerDisconnected
        );
        assert_eq!(
            crate::endpoint::h3_connection_close_reason(&quinn::ConnectionError::LocallyClosed),
            RequestCancellationReason::ServerShutdown
        );
    }

    #[test]
    fn alt_svc_advertisement_uses_resolved_port_and_denylist() {
        ...
        let server_config = RuntimeConfig::builder()
            ...
            .build()
            .unwrap();
        let h3_config = crate::config::Http3Config {
            enabled: true,
        ...
        let response = Response::builder()
            ...
            .unwrap();
        let response = apply_alt_svc(response, &server_config, &h3_config);
        let values: Vec<_> = response
            .headers()
            .iter()
            .filter(|f| f.name.as_str().eq_ignore_ascii_case("alt-svc"))
            .collect();
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].value.to_str().unwrap(), "h3=\":9443\"; ma=86400");
    }
}
