...
/// Small executor-neutral notification primitive.
///
/// Tokio's `Notify` is deliberately not used here: lifecycle values are part
/// of the dependency-free canonical crate. A generation counter closes the
/// check/register race, while registered wakers provide prompt wake-up on any
/// executor.
#[derive(Debug)]
struct AsyncNotify {
...
pub(crate) struct RequestShared {
    ...
    body_notify: AsyncNotify,
    /// Notified when cancellation fires.
    cancel_notify: AsyncNotify,
}
