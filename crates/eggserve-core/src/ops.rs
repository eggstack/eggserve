...
pub struct CompositeLogSink {
    ...
    /// the historical process-global accounting via [`global_counters`];
    /// per-runtime contexts should prefer
    /// [`CompositeLogSink::with_failure_counters`] or
    /// [`OpsContext::with_sinks`] so a failing sink is counted in the same
    /// context that owns it (Plan 181 Track B2).
