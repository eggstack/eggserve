//! Shared rustls server configuration loading and production TLS identity.
//!
//! The legacy path-based loader (`load_tls_config`) remains for the CLI and
//! the Python compatibility facade (single identity, no client auth). Plan
//! 203 adds an EggServe-owned identity model on top of maintained rustls
//! mechanisms:
//!
//! - [`TlsServerConfigBuilder`] / [`TlsServerConfig`] for multiple SNI
//!   identities (exact + well-defined single-level wildcards, optional
//!   default) with WebPKI client authentication (`Disabled` / `Optional` /
//!   `Required`);
//! - [`TlsReloadHandle`] for atomic reload of new handshakes;
//! - bounded, validated construction (no blocking IO in the handshake path,
//!   no PEM/key bytes in logs, early key/cert pairing checks).
//!
//! EggServe does not accept Python SSL contexts, expose key material, provide
//! certificate management/ACME, watch filesystems, or invoke Python callbacks
//! during verification. Session tickets use rustls defaults (no stateful
//! tickets; see docs) and 0-RTT stays disabled (`max_early_data_size = 0`).
...
///
/// H3 uses a separate TLS 1.3/`h3` configuration (QUIC cannot reuse the TCP
/// H1/H2 ALPN set) and disables 0-RTT. Identity rotation for the QUIC
/// endpoint requires endpoint replacement/drain (see `docs/tls.md`); TCP
/// reload via [`TlsReloadHandle`] does not atomically rotate H3.
#[cfg(feature = "http3")]
