...
//!
//! - **Header order**: [`eggserve_primitives::HeaderBlock`] preserves exact
...
//!   the native [`eggserve_primitives::RequestContext`] wrapper.
//!
...
use eggserve_primitives::authority::{Authority, AuthorityError};
use eggserve_primitives::canonical::{
    Response, ResponseBody, ResponseConstructionError, ResponseStream,
};
use eggserve_primitives::connection_info::ConnectionInfo;
use eggserve_primitives::header_block::{HeaderBlock, HeaderError};
use eggserve_primitives::method::{Method, MethodError};
...
pub fn status_to_http(status: eggserve_primitives::canonical::StatusCode) -> http::StatusCode {
...
) -> Result<eggserve_primitives::canonical::StatusCode, InteropError> {
    eggserve_primitives::canonical::StatusCode::new(status.as_u16())
...
pub fn header_map_to_block(map: &http::HeaderMap) -> Result<HeaderBlock, InteropError> {
    ...
    for (name, value) in map.iter() {
        let cname = eggserve_primitives::header_block::HeaderName::new(name.as_str())
            .map_err(|_| InteropError::InvalidHeader)?;
        let cvalue = eggserve_primitives::header_block::HeaderValue::from_bytes(value.as_bytes())
            .map_err(|_| InteropError::InvalidHeader)?;
...
///
/// This server-owned newtype makes the external `http_body::Body` implementation
    ...
    B::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    let status = eggserve_primitives::canonical::StatusCode::new(parts.status.as_u16())
...
pub fn response_from_bytes(
    status: eggserve_primitives::canonical::StatusCode,
    headers: &HeaderBlock,
    body: Option<Bytes>,
) -> Result<Response, ResponseConstructionError> {
...
pub fn empty_response_from_http(
    ...
    let canonical_status = eggserve_primitives::canonical::StatusCode::new(status.as_u16())
...
mod tests {
    ...
    fn http_request_body_error_display_is_sanitized_and_unwrap_preserves_owner() {
        ...
        assert_eq!(
            body.state(),
            eggserve_primitives::request_body::BodyState::Unread
        );
    }
    ...
    fn status_round_trip() {
        let s = eggserve_primitives::canonical::StatusCode::OK;
