//! Final-boundary response privacy policy (Plan 215: moved from compatibility core).
//!
...
//! `docs/deployment.md`.
//!
//! `eggserve_core::server::response_policy` re-exports this module during the
//! 0.x line; this crate is the single implementation authority.

use std::sync::Arc;
use std::time::SystemTime;

pub use eggserve_primitives::policy::ErrorRepresentationPolicy;
...
pub fn validate_stripped_header_name(name: &str) -> Result<String, String> {
    let canonical = eggserve_primitives::header_block::HeaderName::new(name.to_string())
...
impl ResponsePolicy {
    ...
    /// [`eggserve_primitives::policy::StaticMetadataPolicy::minimal_fingerprint`] (suppress
    ...
    pub fn validate(&self) -> Result<(), String> {
        if let Some(server) = &self.server_identification {
            let canonical = eggserve_primitives::header_block::HeaderValue::new(server.clone())
