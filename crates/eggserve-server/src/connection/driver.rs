//! H1-only driver: Hyper builder, graceful close, outcome classification,
//! and the deadline/select loop (Plan 215: moved from compatibility core).
//!
...
/// [`crate::response_policy::ResponsePolicy`] is the sole `Date`
...
/// Shared H1 connection driver: polls one Hyper connection while enforcing
/// independent deadlines.
///
/// - `connection_total_timeout` — optional hard maximum connection lifetime,
///   never reset (defense in depth); zero disables only this deadline;
...
/// broadcast channel. Used only by
/// [`serve_http1_connection`](super::serve_http1_connection).
    ...
    S::Future: Send + 'static,
{
    ...
    ops.emit(
        ...
        .connection_id(conn_id)
        .field(crate::ops::Field::Str("protocol".into(), "http/1.1".into())),
    );
