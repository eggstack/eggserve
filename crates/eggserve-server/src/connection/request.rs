...
pub(crate) fn wrap_incoming_body(
    ...
    Item = Result<bytes::Bytes, eggserve_primitives::request_body::IncomingError>,
> + Send
       + 'static {
    wrap_incoming_body_with_trailers(body, eggserve_primitives::request_body::new_wire_slot()).0
}
...
pub(crate) fn wrap_incoming_body_with_trailers(
    body: Incoming,
    slot: eggserve_primitives::request_body::WireTrailerSlot,
) -> (
    impl futures_util::Stream<
            Item = Result<bytes::Bytes, eggserve_primitives::request_body::IncomingError>,
        > + Send
        + 'static,
    eggserve_primitives::request_body::WireTrailerSlot,
) {
    ...
    let stream = http_body_util::BodyStream::new(body).filter_map(move |result| {
        ...
        async move {
            match result {
                    ...
                    if frame.is_trailers() {
                        ...
                        *seen_guard = true;
                        ...
                        if seen.lock().map(|g| *g).unwrap_or(false) {
                            ...
                            return Some(Err(eggserve_primitives::request_body::IncomingError(
                ...
                Err(e) => Some(Err(eggserve_primitives::request_body::IncomingError(
                    e.to_string(),
                ))),
            }
        }
...
fn hyper_to_header_block(
    map: &hyper::HeaderMap,
) -> Result<eggserve_primitives::header_block::HeaderBlock, String> {
...
pub(crate) fn convert_request_head(
...
) -> Result<eggserve_primitives::request_head::RequestHead, ServiceError> {
    use eggserve_primitives::authority::Authority;
    ...
    for (name, value) in req.headers().iter() {
        ...
        let header_name = eggserve_primitives::header_block::HeaderName::new(name.as_str())
        ...
        let header_value = eggserve_primitives::header_block::HeaderValue::from_bytes(
            value.as_bytes(),
        )
