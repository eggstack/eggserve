...
//!
//! Request bodies arrive as [`HttpRequestBody`](crate::interop::HttpRequestBody),
//! a server-owned `http_body::Body` view over the canonical body, preserving
    ...
    S::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    fn call(
        ...
        Box<
            dyn Future<Output = Result<eggserve_primitives::canonical::Response, ServiceError>>
                + Send
                + '_,
        >,
    > {
        ...
        *head_req.extensions_mut() = extensions;
        ...
        let fut = async move {
            let (method, target, version, headers, authority) =
                crate::interop::request_head_from_http(&head_req)
                    .map_err(TowerAdapterError::Interop)?;
            let connection = head_req
                .extensions()
                .get::<crate::interop::ConnectionInfoExt>()
                .map(|e| e.0.clone())
                .unwrap_or_else(|| {
                    eggserve_primitives::ConnectionInfo::without_socket_addrs(
                        eggserve_primitives::Scheme::Http,
                        None,
                    )
                });
            ...
            let lifecycle = head_req
                .extensions()
                .get::<crate::interop::LifecycleExt>()
                .map(|e| e.0.clone())
                .unwrap_or_else(|| body.lifecycle());
            let is_head = method.is_head();
            let head = eggserve_primitives::RequestHead::new_with_authority(
            ...
            let hyper_resp = crate::adapters::to_hyper_response(normalized)
                .map_err(TowerAdapterError::Construction)?;
...
pub enum TowerAdapterError {
    ...
    Construction(eggserve_primitives::canonical::ResponseConstructionError),
}
