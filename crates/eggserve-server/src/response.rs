...
pub(crate) fn canonical_error_owned_with_policy(
    ...
    let code = eggserve_primitives::canonical::StatusCode::new(status.as_u16())
    ...
    let mut headers = eggserve_primitives::header_block::HeaderBlock::new();
    ...
    if policy == eggserve_primitives::policy::ErrorRepresentationPolicy::Minimal {
        headers
    ...
    eggserve_primitives::canonical::normalize_metadata(
        code,
        &mut headers,
    ...
    for field in headers.iter() {
        let name = hyper::header::HeaderName::from_bytes(field.name.as_str().as_bytes())
            .map_err(|_| {
                eggserve_primitives::canonical::ResponseConstructionError::InvalidHeader(
...
pub(crate) fn runtime_error_with_policy(
    ...
    let canonical_status = eggserve_primitives::canonical::StatusCode::new(status.as_u16())
        .unwrap_or(eggserve_primitives::canonical::StatusCode::INTERNAL_SERVER_ERROR);
    let canonical = eggserve_primitives::canonical::runtime_error_with_policy(
        canonical_status,
        is_head,
        policy,
    );
