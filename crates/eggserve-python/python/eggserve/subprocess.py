"""Optional subprocess/CLI convenience API (canonical owner).

This module owns the subprocess convenience implementation:
``StaticPolicy``, ``ServeConfig``, ``ServerProcess``, ``serve_directory``,
and the private ``_parse_bind`` / ``_config_to_argv`` bridge helpers.

``eggserve.server`` retains compatibility re-exports of these names but is
otherwise focused on the six-class ``http.server``-shaped facade. Import
from here for subprocess lifecycle use; the top-level
``eggserve.serve_directory`` is a convenience re-export of
