...
| `response_stream.rs` | `primitives/response_stream.rs` | `ResponseStream`, `ResponseStreamError`, `MAX_RESPONSE_STREAM_CHUNK_BYTES` — transport-independent streaming bodies with one terminal trailer source (`with_trailers`, `with_known_length_and_trailers`; Plan 299 declaration-aware `with_declared_trailers`, `with_known_length_and_declared_trailers` for H1 wire delivery) |
...
| `request_context.rs` | `primitives/request_context.rs` | `RequestContext` — typed context/capability container (Plans 197–199): `connection()` + `lifecycle()` + bounded `interim()` (`InterimSender`) + validated intent `tunnel_request()` (0.1 `take_tunnel()` removed in 0.2, use `Service::call_with_tunnel`); `connection()` carries Plan 202 effective fields when trusted |
| `trailers.rs` | `primitives/trailers.rs` | `Trailers`, `TrailerLimits`, `TrailerValidationError`, `TrailerDeclaration` (Plan 299 head-time names-only declaration for H1 wire delivery) — distinct terminal metadata, denylist + count/byte limits, one validator for H1/H2/H3 |
