...
- Plan 078 corrects custom-service ownership: `build_with_service` removed (custom services must use `start_with_service()`), real peer/local addresses threaded through connection pipeline, Python `Request` now exposes `remote_addr`, `local_addr`, and `scheme` from actual transport metadata.
