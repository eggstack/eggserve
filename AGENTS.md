...
- **Service body policy** — `Service::request_body_policy(&RequestHead)` declares the preferred body policy (Reject/Buffer/Stream) per-request (method-aware). The runtime enforces the global ceiling (`max_request_body_bytes`) and service-specific limits may only lower it. Default is `Reject`.
