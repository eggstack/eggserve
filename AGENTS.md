...
- Qualification evidence (Plans 168/170/227–231): deterministic track suites and the manual performance-evidence matrix are mapped in `architecture/testing-and-conformance.md`; methodology, regression policy, and claims policy live in `benchmarks/README.md`, with machine-readable results under `benchmarks/168-qualification/`, `benchmarks/170-closure/`, `benchmarks/227-current-head/`, and `benchmarks/231-optimization-closure/`. The current file-stream default is 128 KiB, selected by Plan 229 evidence and still bounded by `max_file_streams * stream_chunk_size`; direct H1 write timeout progress is socket-write based, while H2/H3 retain their protocol-specific producer semantics. No absolute RPS/latency gates in CI; every performance/release claim names a profile + evidence.
