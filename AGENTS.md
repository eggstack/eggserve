...
- **Outbound response conversion boundary** — `eggserve_server::adapters::to_hyper_response()` is the direct low-level adapter with an opaque `http_body::Body<Data = Bytes, Error = io::Error>` body; downstream code must not name `BoxBody`/`UnsyncBoxBody`. The compatibility `primitives::to_hyper_response()` keeps its own implementation over compatibility types until Plan 216 unifies Request types; behavior parity is covered by the direct-vs-compatibility H1 suite. The semaphore-aware helper is runtime-internal. The current `main` contract is the intentional `0.1.x` → `0.2.0` pre-1.0 transition documented in `docs/migration-guide.md`.
