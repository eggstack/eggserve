...
- **Production profiles** — `release/support-profiles.toml` is the single source of truth for production deployment profiles. Every production claim must name a profile. Profiles are: unix-reverse-proxy (candidate, Plan 089 gates defined), unix-direct-https (candidate, Plan 089 gates defined), windows-reverse-proxy (candidate), windows-direct-https (functional), local-development (supported-hardened), windows-functional (functional), link-following-compat (functional).
...
- Plan 078 corrects custom-service ownership: `build_with_service` removed (custom services must use `start_with_service()`), real peer/local addresses threaded through connection pipeline, Python `Request` now exposes `remote_addr`, `local_addr`, and `scheme` from actual transport metadata. ADR-003 documents the ownership model decision. TLS info (protocol version, SNI server name) extracted from completed TLS sessions.
- Plan 079 corrects request-body rejection: `RequestBodyPolicy::Reject` returns 413 without invoking the service, preventing handler side effects for rejected requests. `IncompleteBodyPolicy::Drain` removed (close-only) because active drain is not safely implementable after service invocation. `Expect: 100-continue` rejected early for bodies that will be rejected by policy. Pre-service rejection drain bounded to 5 seconds.
- Plan 080 corrects configuration authority: `Limits::validate()` added with `LimitsError` type, `Limits` gains `handler_timeout` and `body_read_timeout` fields, `try_from_serve_config()` replaces infallible `From` (validates limits before runtime construction), `Args::limits()` returns `Result` with validation, `StaticServiceBuilder::build()` validates file-stream limits, Python defaults aligned with Rust (max_connections=64, max_file_streams=32, connection_total_timeout=60s), CLI adds `--handler-timeout` and `--body-read-timeout` flags, configuration drift tests added. All construction paths now reject zero concurrency/timeouts with structured errors. Configuration inventory (`architecture/configuration.md`), ownership model, and machine-readable schema (`release/config-schema.toml`) established. CLI integration tests prove validation error messages. Python validation error tests added.
- Plan 081 unifies static-file and directory-index response paths: introduces `StaticRequestInput` (bundles method + conditional headers) and `serve_resolved_file()` (single entry point for both direct-file and directory-index routes), eliminating semantic drift between equivalent resources served via different URL forms.
...
- Plan 085 implements Windows handle-relative directory enumeration. `NtQueryDirectoryFile` with `FileIdBothDirectoryInfo` replaces the path-based `GetFinalPathNameByHandleW` + `FindFirstFileW` fallback. A safe bounded parser (`parse_directory_buffer`) validates variable-length `FILE_ID_BOTH_DIR_INFO` records. `DirectoryEntryRecord` provides a platform-neutral entry type. Adversarial qualification test scaffold established (Plan 086, 114 tests). Independent safety review and profile promotion decision awaited.
- Plan 086 establishes Windows adversarial filesystem qualification. Test scaffold covers reparse-point denial matrix, namespace normalization, concurrent mutation races, root identity, file validators, ACL/sharing behavior, resource stability, and installed artifact parity. 114 tests in `crates/eggserve-core/tests/windows_plan086.rs`. Release gates added to `release/criteria.toml`. Independent safety review and profile promotion decision awaited. Dedicated Windows VM required for Developer Mode symlink/junction fixtures.
...
- Plan 088 establishes streaming allocation and buffer performance qualification. Extracts `DEFAULT_CHUNK_SIZE` constant (8 KiB) for file streaming, adds `stream_chunk_size` to `Limits` (validates 64 B–1 MiB) and wires it into file streaming functions, optimizes `normalize_metadata` header filtering to use in-place `retain` instead of clone+rebuild, adds 27 buffer-qualification tests (exact range boundaries, chunk-crossing ranges, buffer isolation, zero-length files, client disconnect release, forced shutdown release, concurrent exhaustion, HEAD non-acquisition, configurable chunk sizes), adds representative file-serving benchmarks (`benches/file_serving.rs`) covering GET/HEAD by file size, range requests, conditional requests, error paths, directory listing, keep-alive sequences, body consumption, and chunk count regimes, captures baseline results with environment metadata (`benchmarks/088-baseline/results.json`), completes allocation audit and range/seek/accept-loop/TLS/directory-listing/request-body audits, adds 9 performance regression gates to `release/criteria.toml`, and updates architecture docs with baseline performance table.
- Plan 089 closes the production-readiness roadmap. Adds 13 release gates to `release/criteria.toml`: proxy Caddy/nginx interop, proxy desync corpus, native TLS abuse/limits, stateful live-socket fuzz replay, Unix filesystem race qualification, fault injection, 24-hour soak (unix-reverse-proxy and unix-direct-https profiles), installed binary validation, SBOM/provenance, independent security review, and production profile promotion decisions. Updates `release/support-profiles.toml` with Plan 089 gate requirements for unix-reverse-proxy and unix-direct-https profiles. Creates release operator runbook (`docs/release-runbook.md`). Gate infrastructure is implemented; external qualification evidence collection (proxy runners, TLS abuse harness, 24h soak, independent reviewer) is pending.
- Plan 090 establishes the corrective evidence and production qualification closure. Adds implementation/evidence state model to `release/corrective-findings.toml` (schema v2 with `implementation_status`, `evidence_status`, `implementation_sha`, `evidence_sha`, `required_gates`, `blocking_reason`, `profile_impact`, `review_status`). Removes panic-capable `PinnedRoot::clone()` — `PinnedRoot` no longer implements `Clone`; use `Arc<PinnedRoot>` for shared ownership. `SecureRoot` now uses `Arc<PinnedRoot>` internally. Adds `PinnedRoot::try_clone()` for fallible duplication. Adds `candidate` and `validate-all` commands to `scripts/release_criteria.py` for profile-aware evidence validation with fail-closed finding registry checks. Reconciles TE+CL documentation to accurately describe parser-boundary behavior (Hyper may normalize headers before eggserve's validator runs). Adds COR-019 finding for PinnedRoot panic fix. All 19 corrective findings now have implementation/evidence separation. Profiles remain correctly unpromoted pending qualification evidence.
- Corrective Closure Pass (Phases 31–35) closes repository-hygiene and verification gaps: tightens 16 permissive wire tests to normative single-outcome assertions, fixes silent `into_body()` error swallowing in service layer (now returns 500), adds API stability mechanical checks (28 tests), adds 10 missing corpus replay tests (covering header_block, header_name, header_value, method, request_head, request_body, response_builder, status_code, normalize_response, content_length_reconciliation), documents filesystem race test taxonomy with 4 categories and adds 2 bounded concurrent swap stress tests.
