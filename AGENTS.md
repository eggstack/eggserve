...
- Plans 288–291 extend only the direct H1 boundary: the old 4 MiB / 10,000 parser values are guidance rather than validator maxima (8192-byte Hyper minimum and positive header count remain); aggregate post-parse header bytes can be externalized independently; successful service responses can opt into per-response Date/Server ownership with explicit runtime/service provenance. Keep high-level defaults and H2/H3 unchanged, keep runtime failures under `ResponsePolicy`, and do not claim registry availability until Plan 291's registry consumers pass.
