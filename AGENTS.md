...
- **Protocol-neutral request preparation (Plans 184–186)** — `RequestTarget::parse()` is the sole HTTP target-form classifier. Static/runtime code passes its validated `path()` to `ConfinedPath::from_path_component()`; the raw `ConfinedPath::parse()` API remains a compatibility adapter. `Authority` maps validated HTTP/1 `Host` or HTTP/2 authority metadata without exposing pseudo-header names. Default builds remain HTTP/1-only; the opt-in `http2` feature adds bounded H1/H2 Rust serving, while Python remains HTTP/1.1-shaped. Plan 186 closes H2 as experimental because broad independent-client/platform evidence and a public per-stream reset hook are still missing.
