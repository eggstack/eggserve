...
- Plan 080 corrects configuration authority: `RuntimeConfigBuilder::build()` returns `Result<RuntimeConfig, ServerError>` instead of panicking on invalid values, `Limits::validate()` added, `Limits` gains `handler_timeout` and `body_read_timeout` fields, Python defaults aligned with Rust (max_connections=64, max_file_streams=32, connection_total_timeout=60s), `From<&ServeConfig>` uses `Limits` fields instead of hardcoding, CLI adds `--handler-timeout` and `--body-read-timeout` flags, configuration drift tests added.
