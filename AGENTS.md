...
superseding deferred Plan 176: one-shot transport-backed `TunnelCapability`
(H1 `Upgrade`, `CONNECT`, H2/H3 Extended `CONNECT`; H3 generic `:protocol`
blocked by `h3` 0.0.8), `accept` returns a handshake `Response` (`101` H1 /
`200` otherwise, runtime owns framing, no raw socket) plus bounded
single-owner `TunnelIo`; denial stays ordinary HTTP; WebSocket framing stays
downstream (see `tunnel_upgrade.rs`). Plan 216 moves tunnel authority to the
direct crates without a new service model: neutral intent vocabulary
(`TunnelKind`/`ProtocolName`/`TunnelRequest`/`TunnelError`, classifiers,
handshake validator) lives once in `eggserve-primitives::tunnel`
(Hyper/Tokio-free); transport execution (`TunnelCapability`/`TunnelIo`/
acceptance state/H1 detection/bounded bridging/shared `run_tunnel`) lives
once in `eggserve-server::tunnel`; compatibility H1/H2 validate through the
shared helpers and run the shared future (0.1 thin `take_tunnel` wrapper removed in 0.2,
`server/connection/tunnel.rs` deleted; H3 keeps its stream bridging under
Plan 213). Direct services use additive `Service::call_with_tunnel`
(default drops → ordinary denial); handlers own only IO
...
`conformance/http3_qualification.toml`). Plan 220 moves the H3/QUIC adapter
implementation into `eggserve-h3` (endpoint, request, response, tunnel, QUIC
assembly, `Http3Config` authority; narrow adapter over primitives/server;
core keeps a thin facade with no second state machine).
...
ownership rules; `eggserve-core::server` keeps extended
TLS/H2/proxy-accept ownership plus the thin H3 facade with facades over every moved
module. Plan 216 moves tunnel authority to the direct crates; Plan 217
...
`release/plan-248-maintainability-convergence-closure.md`. Plans 249–250 close
the post-closure corrective: compatibility `Auto` classifies before any Hyper
service exists with every H1 path delegating to the single direct authority
(core drives H2 only), and per-connection shutdown is structured under the
...
- **Package roles**: `eggserve-primitives` owns the extracted canonical
  ...
  dependencies; `eggserve-server` owns the mature direct generic H1
  connection runtime (ops, error taxonomy, response policy, shared limit
  authority, service contract shape, connection vocabulary, H1 config/state,
  ...
  planning. `eggserve-core` is the compatibility and composition umbrella
  for Python, H2/H3, tunnel, proxy, and advanced
...
- **Plans 243–247 convergence** — direct server shutdown must use the durable
  ...
  in core unless a new migration plan moves it. Plan 249: compatibility `Auto`
  classifies before any Hyper service exists, core executes H2 only, and
  per-connection shutdown is structured (`run_with_connection_shutdown`);
...
- **Plan 214 extraction parity** — direct crates are the implementation homes
  ...
  direct server owns the mature H1 connection substrate (Plan 215, with a
  direct-vs-compatibility parity suite and topology ownership rules); Plan 216
...
- **Protocol-owned configuration** — Plan 179 shared defaults remain in `runtime_limits`; H1 fields project onto the direct authority via `RuntimeConfig::direct_h1_config()` with no second defaults table (`server/config/http1.rs` is a retained inventory placeholder with no H1 authority, Plan 249). H2/H3 controls must use protocol-owned projections. H1 runs with `.with_upgrades()` + H2 `enable_connect_protocol()` + H3 `enable_extended_connect(true)` for validated tunnel handshakes (Plan 199); ordinary services pay no upgrade cost (unused capabilities dropped). The workspace MSRV is Rust 1.89; keep dependency updates compatible with the pinned MSRV and supply-chain checks.
...
- **Outbound response conversion boundary** — `eggserve_server::adapters::to_hyper_response()` is the direct low-level adapter with an opaque `http_body::Body<Data = Bytes, Error = io::Error>` body; downstream code must not name `BoxBody`/`UnsyncBoxBody`. The compatibility `primitives::to_hyper_response()` delegates to that single authority over the same canonical types (Plan 217 identity, no second framing implementation); behavior parity is covered by the direct-vs-compatibility H1 suite plus the direct-service convergence fixture plus both tunnel suites (`eggserve-server/tests/tunnel_upgrade.rs`, `eggserve-core/tests/tunnel_upgrade.rs`). The semaphore-aware helper is runtime-internal. The `0.2.0` line carries the intentional `0.1.x` → `0.2.0` pre-1.0 transition documented in `docs/migration-guide.md` (Plan 226 executed the version move; never publish this line as `0.1.x`).
...
- **HTTP/3 adapter extraction (Plan 220)** — `eggserve-h3` owns the H3/QUIC adapter implementation (endpoint, request, response, tunnel, QUIC assembly, `Http3Config` authority; narrow adapter over primitives/server; core keeps a thin facade with no second state machine).
...
- Qualification evidence (Plans 168/170/227–241): deterministic track suites and the manual performance-evidence matrix are mapped in `architecture/testing-and-conformance.md`; methodology, regression policy, and claims policy live in `benchmarks/README.md`, with machine-readable results under `benchmarks/168-qualification/`, `benchmarks/170-closure/`, `benchmarks/227-current-head/`, `benchmarks/231-optimization-closure/`, `benchmarks/232-corrective/`, `benchmarks/233-evidence-polish/`, `benchmarks/234-fixed-cost-baseline/`, `benchmarks/240-fixed-cost-closure/`, and `benchmarks/241-fixed-cost-evidence-corrective/`. Plans 234–240 retain canonical request-target/header/body representations, hardened root-FD/path fast paths, generic H1 dispatch, zero-tunnel checks, and lazy Python compatibility views without changing public behavior; Plan 238 is NO-GO and the Python producer-thread redesign is DEFER. Plan 241 completes the missing custom/path/TLS/installed-wheel Python/slow-stream/syscall evidence without production changes; TLS/metadata variance is classified explicitly and unavailable peer-chain exposure is not inferred. The current file-stream default is 128 KiB, retained by Plan 232's live 64/128 KiB throughput/resource comparison and bounded by `max_file_streams * stream_chunk_size`; `read_file_chunk` takes an explicit logical target and never uses allocator capacity as a response-length authority. Manual performance qualification must retain compact per-trial JSON rather than only aggregate reductions of discarded captures. Direct H1 write timeout progress is socket-write based, while H2/H3 retain their protocol-specific producer semantics. No absolute RPS/latency gates in CI; every performance/release claim names a profile + evidence.
