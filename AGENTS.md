...
- Qualification evidence (Plans 168/170/227–232): deterministic track suites and the manual performance-evidence matrix are mapped in `architecture/testing-and-conformance.md`; methodology, regression policy, and claims policy live in `benchmarks/README.md`, with machine-readable results under `benchmarks/168-qualification/`, `benchmarks/170-closure/`, `benchmarks/227-current-head/`, `benchmarks/231-optimization-closure/`, and `benchmarks/232-corrective/`. The current file-stream default is 128 KiB, retained by Plan 232's live 64/128 KiB throughput/resource comparison and bounded by `max_file_streams * stream_chunk_size`; `read_file_chunk` takes an explicit logical target and never uses allocator capacity as a response-length authority. Direct H1 write timeout progress is socket-write based, while H2/H3 retain their protocol-specific producer semantics. No absolute RPS/latency gates in CI; every performance/release claim names a profile + evidence.
