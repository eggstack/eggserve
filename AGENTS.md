...
- **HTTP/3 boundary (Plan 187)** — `RuntimeConfig::http3` owns explicit QUIC/H3 windows, stream/handshake/field-section/send-buffer/idle/retry limits. `ServerBuilder::http3_identity` supplies PEM paths without exposing Quinn/rustls types; the runtime creates a separate TLS 1.3 QUIC config and starts UDP atomically after TCP bind. H3 request streams map to the canonical `Request`/`Service`/`Response` path, including body limits, strict content length, per-stream reset, GOAWAY drain, and runtime-owned `Alt-Svc`. H3 is experimental; Plan 188 owns independent-client interoperability, adversarial resource tests, platform evidence, and support-tier declaration.
