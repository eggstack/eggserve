...
- **Plan 189 multiprotocol correctness** — H1 Reject remains framing based; H2 Reject uses Hyper's public `Incoming::is_end_stream()` and H3 performs one bounded receive probe when `Content-Length` is absent/zero, so DATA cannot reach a Reject service. Shared `RequestBody` consumption checks both under- and over-declared lengths. H3 reuses the connection lifecycle registry for peer/shutdown cancellation and stream-local failure handling. Runtime error status/reason/body construction is canonical across H1/H2/H3. H2 response timeout accounting observes application-body poll progress only, not guaranteed wire progress; no public Hyper stream reset is assumed, so H2/H3 remain experimental.
...
- **HTTP/3 boundary (Plans 187–188)** — `RuntimeConfig::http3` owns explicit QUIC/H3 windows, stream/handshake/field-section/send-buffer/idle/retry limits. `ServerBuilder::http3_identity` supplies PEM paths without exposing Quinn/rustls types; the runtime creates a separate TLS 1.3 QUIC config and starts UDP atomically after TCP bind. H3 request streams map to the canonical `Request`/`Service`/`Response` path, including body limits, strict content length, per-stream reset, GOAWAY drain, and runtime-owned `Alt-Svc`. H3 remains experimental after Plan 188 closure because this environment lacks independent-client, adversarial-wire, and cross-platform runtime evidence; see `release/plan-188-http3-qualification.md`.
