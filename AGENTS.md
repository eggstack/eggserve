...
shared helpers and run the shared future (thin `take_tunnel` wrapper,
...
TLS/H2/H3/tunnel/proxy-accept ownership with facades over every moved
module. Unified `Service` identity and tunnel-acceptance convergence are
explicit Plan 216 input (see `release/plan-215-direct-runtime-parity.md`).
...
- **Plan 214 extraction parity** — direct crates are the implementation homes
  ...
  direct-vs-compatibility parity suite and topology ownership rules); keep
  H2/tunnel/`Service`-identity/TLS-compat work in core until Plans 216/217
  land their own extraction with parity evidence.
