...
- **Plan 108 runtime closure** — Plan 108 completed the corrective pass for planner metadata attachment, rootless Python custom startup, canonical static planning, conditional Stream connection closure, and distribution-profile evidence. The production `Server` path owns one shared `RuntimeState` file-stream pool. The pre-runtime `eggserve_core::service` adapter and its compatibility admission handle remain only for existing alpha/in-repository consumers and must not be treated as a production server path.
