# EggServe Planning and Agent-Handoff Process

Status: normative planning governance (Plan 292; adapted from CodeGG `003-planning-process.md`).

This document defines how EggServe's long-term architecture is translated into actionable work without allowing short-lived implementation details to destabilize the canonical specification, terminology, or master roadmap.

The keywords MUST, MUST NOT, REQUIRED, SHOULD, SHOULD NOT, and MAY are normative.

## 1. Purpose

EggServe requires two distinct planning horizons:

1. **Long-term planning** defines product identity, crate-ownership boundaries, architectural invariants, capability dependencies, non-goals, and end-state acceptance criteria.
...
Accepted ADRs MUST NOT be rewritten to conceal history. A later decision supersedes the prior ADR and links to it. The existing `architecture/adr-*.md` records stay where they are; new ADRs live in `plans/adrs/` and link back rather than rewriting.
...
- subsystem purpose and ownership boundary (which crates own it; checked by `scripts/check-crate-topology.py`);
...
- focused and broad verification commands (`./scripts/verify.sh fast` scope first; `full`/`deep` where the plan requires);
...
A closure record determines whether a milestone is actually complete. New milestones record closure in `closure/<subsystem>/NNN-status.md` (legacy milestones keep their `release/plan-*.md` records; link them, do not duplicate).

It MUST include:

- implementation commits or pull requests;
- requirement-to-evidence matrix;
- tests and guards run, with outcomes (exact commands; never claim unrun verification);
...
A code commit message saying a plan is closed is not sufficient closure evidence. For publication milestones, registry-only consumer proof and exact-SHA hosted CI (per Plans 226/272/275/286/291 precedent) remain REQUIRED where the plan states them.
...
Canonical long-term documents and accepted ADRs MUST NOT be archived merely because their initial implementation completed. Legacy flat `plans/NNN-*.md` files are archived in place under Plan 292 until a mechanical move happens.
...
- loopback bind by default; no symlinks/dotfiles/listings unless explicitly opted in;
- no serving outside the configured root (descriptor/handle-relative confinement);
- `eggserve-static` owns all static/path/filesystem decisions; `eggserve-server` owns the H1 runtime;
- H1 + canonical `primitives` supported; `server`/H2/H3/tunnel/trailer/adapter/listener/proxy/TLS-identity/async-Python remain experimental.

Invariant work normally requires static guards, conformance corpora, property/fuzz evidence, or topology-gate proof.
...
- confined static file serving with correct conditional/range/ETag semantics;
- static-only CLI with the `[OPTIONS] [PORT] [DIRECTORY]` grammar;
- six-class `http.server`-shaped Python facade; handler-only `lowlevel` substrate.
...
- canonical `primitives` request/response/body/lifecycle model;
- direct H1 connection driver and admission pool;
- neutral `eggnet-tls` identity/trust/reload substrate.
...
Polish SHOULD normally follow functional and correctness closure unless it removes an immediate safety or usability blocker. Benchmark claims name a `docs/deployment.md` profile plus retained evidence; absolute RPS/latency never gates routine CI.
...
- **operational** — implementation can land, but deployment or release depends on external evidence (e.g. registry publication, platform qualification).
...
A milestone is too large when it combines several independently releasable capability boundaries, requires unrelated migrations, or contains several unresolved architecture decisions.
...
1. `docs/` normative user contracts + `architecture/` deep dives (for current behavior);
2. canonical long-term specification and terminology;
3. accepted ADRs;
4. subsystem roadmap;
5. milestone implementation plan;
6. current repository evidence (executable source wins when docs conflict).
...
- respect crate-topology ownership (`eggserve-static` for path/FS, `eggserve-server` for H1 runtime);
- never use `println!`/`eprintln!` in library code (emit via `OpsContext`);
- update tests, conformance corpora, and architecture docs with code;
...
5. explicit ownership and invariants (crate authority + topology impact);
6. migration and compatibility effects;
7. concurrency, cancellation, restart, and failure semantics;
8. security and authorization effects (safe defaults, confinement, `docs/threat-model.md`);
9. required test and static-guard evidence (`verify.sh`, topology gate, conformance corpora, supply-chain where applicable);
