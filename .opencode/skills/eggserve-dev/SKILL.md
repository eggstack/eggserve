...
- **Qualification evidence (Plans 168/170/227–231)** — These plans are qualification phases: deterministic suites per track plus manual, same-machine performance evidence (see `architecture/testing-and-conformance.md`), never absolute-timing CI gates. Plan 229's current file-stream default is 128 KiB, bounded by `max_file_streams * stream_chunk_size`; direct H1 write timeout progress is actual socket-write progress, while H2/H3 retain protocol-specific producer semantics. Benchmark methodology, regression policy, and claims policy live in `benchmarks/README.md`; machine-readable results are in `benchmarks/088-baseline/results.json`, `benchmarks/168-qualification/results.json`, `benchmarks/170-closure/results.json`, `benchmarks/227-current-head/`, and `benchmarks/231-optimization-closure/`. Every performance/release claim must name a profile + workload + evidence; forbidden claims (edge-proxy parity, DDoS resistance, un-fingerprintability, ASGI/WSGI parity, HTTP/2/3, universal superiority headlines) stay forbidden.
