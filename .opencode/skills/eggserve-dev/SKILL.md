...
Other directories: `architecture/` (deep-dive docs), `docs/` (reference docs), `plans/` (000–099 plus roadmap), `examples/`, `fuzz/`.
...
- **Plan status** — Plans 000–100 are implementation-complete. Plan 091 defines the current CI, verification, and manual release policy, superseding the prior evidence/qualification framework. Plans 092–093 closed the Python installed-wheel and test-reliability gaps; Plans 096–100 define the bounded Python `http.server` façade, static compatibility, TLS classes, narrowed namespaces, and corrective closure. Plan 101 owns final verification and hosted closure evidence. Historical plans are in `plans/`.
