...
- `server.md` — runtime service boundary, Server, Service trait, StaticService
...
- **`server` module types** — `eggserve-core::server` provides the runtime service boundary for embedding: `Server` (entry point), `ServerBuilder` (configured builder), `ServerHandle` (control handle), `RuntimeConfig` (transport config, separate from `ServeConfig`), `Service` trait (transport-independent service abstraction), `service_fn` (create service from closure), `ServiceError` (per-request errors), `ServerError` (lifecycle errors), `StaticService` (hardened static file service), `StaticServiceBuilder`. The module is experimental; API may change.
