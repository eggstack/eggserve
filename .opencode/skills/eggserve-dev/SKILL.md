...
- `crates/eggserve-server/` — mature H1 connection runtime and transport
  boundary (Plan 215 implementation home; tunnel/H2 stay in core)
...
python3 scripts/check-crate-topology.py                     # Plan 211–215 ownership/topology gate
...
- **Plan 214 extraction parity** — `eggserve-primitives` owns the extracted
  ...
  own the boundary; keep H2/tunnel/`Service`-identity/TLS-compat work in
  core until Plans 216/217 land their own extraction with parity evidence.
  (Plan 216 tunnel: neutral vocabulary in `eggserve-primitives::tunnel`,
  execution in `eggserve-server::tunnel`, compat H1/H2 delegate with a thin
  `take_tunnel` wrapper; handlers own only `TunnelIo`, capture the
  lifecycle.)
  Run
...
- **Public Hyper boundary** — Canonical application-facing request/response types, `Service`, and the caller-owned connection driver are Hyper-free. `eggserve_server::adapters::to_hyper_response()` is the direct low-level adapter (the compatibility `primitives::to_hyper_response()` keeps its own implementation over compatibility types; Plan 216 unifies tunnel vocabulary/execution, not Request types; parity covered by the H1 suite plus both tunnel suites); the outbound adapter returns an opaque `http_body::Body` and must not expose `BoxBody`/`UnsyncBoxBody` as a stable contract. The current `main` change is the documented pre-1.0 `0.1.x` → `0.2.0` transition.
