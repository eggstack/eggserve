...
- **Plan status** — Plans 000–085 are complete. Plan 055 verifies Milestone 3 final state. Plan 059 closes Milestone 4. Plan 085 implements Windows handle-relative directory enumeration via `NtQueryDirectoryFile`, replacing the path-based fallback. Reparse-point hardening qualification remains deferred (Plan 086). Production profiles are machine-readable in `release/support-profiles.toml`.
