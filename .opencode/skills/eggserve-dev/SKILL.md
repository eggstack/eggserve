...
Hyper's 8192-byte minimum and require a positive header count); aggregate
post-parse header bytes can be externalized independently; successful service
responses can opt into per-response Date/Server ownership. Carry explicit
...
- **Ecosystem interop (Plans 200/274/276)** — optional `eggserve-server/http-interop` (`eggserve_server::interop`: loss-aware `http` conversions, server-owned `HttpRequestBody` adapter over canonical `RequestBody`, `response_from_http_body` framing-authoritative) and `eggserve-server/tower` (`eggserve_server::tower`: per-request clones driving `poll_ready`, adapter-local ready). Core forwards the features and keeps its old paths as facades. Header cross-name order does not round-trip; opaque values use `from_bytes`; interim/tunnel never enter `Extensions`; middleware runs after parsing/validation, before normalization (see `docs/http-interop.md`). Never add Tower traits to default builds.
...
- **Python wheel support** — GIL-enabled CPython 3.11–3.15 via one `cp311-abi3` wheel per platform (build-once/test-many ABI proof on 3.11–3.15; no per-minor wheels; free-threaded owned by Plan 267). Routine CI builds and tests the Linux wheel; release targets 10 required platforms from `release/wheel-matrix.toml` (manylinux x86_64/aarch64/armv7, musllinux x86_64/aarch64/armv7, macOS x86_64/arm64, Windows x86_64/arm64) plus declared i686/win32 candidates. Plan 268: `manylinux` baseline vs `--compatibility pypi` are separate controls; cross-built AArch64 glibc/musl + Windows ARM64 are deferred to native qualifiers gating aggregation (AArch64 musl via Alpine container); ARMv7 via matching QEMU userspace with `sh`; 3.15 lanes use prerelease until final. `scripts/test-python-wheel.sh` has `MODE=full` (primary interpreter suite) and `MODE=abi-smoke` (cross-interpreter lane) with `WHEEL_PATH` reuse; `scripts/qualify-python-wheel-target.sh` is the rootless real-device path.
