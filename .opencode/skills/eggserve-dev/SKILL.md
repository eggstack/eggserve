...
- **Canonical response normalization** — All response producers converge on `primitives::canonical::normalize_response()`. The function applies HEAD suppression, body-forbidden enforcement (1xx/204/304), hop-by-hop stripping, and content-length computation. `to_hyper_response()` converts to Hyper after normalization. Python handler responses use this path for non-file bodies.
