...
- **Plan status** — Plan 108 is historical; Plan 109 is the active bounded correction. Do not claim final admission ownership, exact Stream wire proof, build-time static-service consumption, or corrected distribution evidence until Plan 109 closes. The pre-runtime `service` module is a deprecated delegating compatibility adapter requiring an explicit runtime context; production servers use the shared `RuntimeState` admission pool.
