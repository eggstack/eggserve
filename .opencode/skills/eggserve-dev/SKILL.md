...
- **Qualification evidence (Plan 168)** — Plan 168 is a qualification phase: deterministic suites per track (see `architecture/testing-and-conformance.md` mapping), never absolute-timing CI gates. Benchmark methodology, regression policy, and the claims policy live in `benchmarks/README.md`; machine-readable results in `benchmarks/088-baseline/results.json` and `benchmarks/168-qualification/results.json`. Every performance/release claim must name a profile + evidence; forbidden claims (edge-proxy parity, DDoS resistance, un-fingerprintability, ASGI/WSGI parity, HTTP/2/3, superiority headlines) stay forbidden.
