...
- **Progress definition**: H1 observes forward socket bytes through `ProgressIo`; direct H1 producer polls do not extend this timer. H2 observes application-body producer/poll progress per response; H3 observes meaningful producer progress plus bounded send calls (Plan 194). H2 producer progress is not a guarantee that Hyper has advanced stream flow control or put bytes on the wire.
