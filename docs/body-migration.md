...
- **TE+CL handling**: when both `Transfer-Encoding` and `Content-Length` survive to eggserve's validator, the request is rejected with 400 before the service is called. Under Hyper 1.11 a lone `Content-Length` alongside `Transfer-Encoding` is discarded during parsing (`Transfer-Encoding` wins per RFC 9112 §6.1). Duplicate Content-Length fields are rejected by Hyper's decoder. This applies to all methods, not just body-forbidden ones.
