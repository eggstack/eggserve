...
| `http3` | No | Experimental bounded native Rust HTTP/3/QUIC runtime; Plans 188, 190, 192, 193, 194, 195, and 213 close at the experimental tier — Plan 213 isolates the direct H3/QUIC package boundary and records deterministic, manual, and blocked qualification evidence; Plan 192 is `BLOCKED` on the latest released stack (`h3` 0.0.8 / `h3-quinn` 0.0.10 / Quinn 0.11.11) with upstream `hyperium/h3#338` unfixed and the `#262` stream-drop remainder open, plus incomplete independent-client, adversarial-wire, and cross-platform runtime evidence; Plan 193 (2026-09-10) closed at preflight without entering promotion qualification (unmet Plan 192 prerequisite) after re-checking the unchanged candidate and inventorying missing evidence; Plan 194 bounds the H3 response-producer wait with an absolute no-progress deadline (empty chunks are not progress) without changing the tier; Plan 195 correctively qualifies that bound (shutdown-race and observability regressions) without changing the tier; not enabled by the Python compatibility facade |
...
The `server` module provides a reusable, transport-owning HTTP runtime for embedding. It owns the TCP accept loop, connection management, optional TLS (feature-gated), and HTTP/1 connection handling by default. With the experimental `http2` feature, native Rust callers may also use bounded H2 prior knowledge and TLS ALPN through the same canonical service pipeline. Downstream projects implement the `Service` trait and provide it to `Server`; the runtime handles transport concerns. Plans 186, 190, and 191 close H2 qualification as experimental; see the [promotion-attempt record](../release/plan-191-http2-supported-tier-qualification.md) for the current evidence boundary.
...
| `ServerHandle` | Control handle: `local_addr()` (panics on Unix-only servers; prefer `tcp_local_addr()`/`endpoints()` there), `shutdown()`, `wait()`, `ready()`, `force_shutdown()`, `state()` |
