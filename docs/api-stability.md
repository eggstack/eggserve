...
All stable canonical request types (`Method`, non-exhaustive `HttpVersion`, `Authority`, `HeaderBlock`, `HeaderName`, `HeaderValue`, `HeaderValueTextError`, `HeaderField`, `RequestTarget`, `RequestHead`, `ConnectionInfo`, `Scheme`, `TlsInfo`, `SocketEndpoints`, `StatusCode`, `ReadOnlyMethod`) implement `Send + Sync`. This means they can be safely shared between threads and sent across thread boundaries. This is a compile-time guarantee enforced by the selected-type assertions in `public_api_consumers`; response values have the separate `Send` guarantee described below.

Error types (`MethodError`, `HttpVersionError`, `AuthorityError`, `HeaderError`, `DuplicateHeaderError`, `RequestTargetError`, `RequestHeadError`, `ResponseConstructionError`, `RequestValidationError`) implement `Send` but not necessarily `Sync`, as they may contain `String` payloads.
...
| `RequestContext` | Experimental | Typed request context / capability container (Plan 197 Track B, extended by Plans 198–199): `connection()` + `lifecycle()` + bounded `interim()` + validated intent `tunnel_request()` (0.1 `take_tunnel()` removed in 0.2); cheap clone never clones the one-shot body; no generic type map, no raw transport handles |
...
| `ServerHandle` | experimental | Control handle: `local_addr()` (panics on Unix-only servers; prefer `tcp_local_addr()`/`endpoints()` there), `shutdown()`, `wait()`, `ready()`, `force_shutdown()`, `state()` |
...
| `Trailers` / `TrailerLimits` | experimental | One terminal trailer block (Plan 198): request `trailers()` / `read_all_with_trailers()` after completion, response `ResponseStream::with_trailers`; denylist + count/byte bounds; `InvalidTrailers` on failure |
| `InterimSender` / `InterimLimits` / `InterimError` | experimental | Bounded request-scoped 1xx sender via `RequestContext::interim()` (Plan 198): 1xx only, no 101/body/trailers, no post-commit, HTTP/1.0 suppressed, single 100 |
| `TunnelRequest` / `TunnelKind` / `ProtocolName` / `TunnelCapability` / `TunnelIo` / `TunnelError` | experimental, `TunnelError` `#[non_exhaustive]` | Generic tunnel handoff (Plan 199, superseding deferred Plan 176; Plan 216 authority: neutral vocabulary in `eggserve-primitives`, execution in `eggserve-server`): one-shot `Service::call_with_tunnel` (single contract, Plan 217), `accept` stages the handshake (`101` H1 / `200` otherwise, runtime owns framing, no raw socket, observed out-of-band); handlers own only `TunnelIo`; denial stays ordinary HTTP; WebSocket framing stays downstream |
| `TrustedProxyConfig` / `IpPrefix` / `ProxyProtocolConfig` / `ForwardedConfig` / `ProxySourceKind` | experimental | Explicit trusted-proxy policy (Plan 202): explicit peers/CIDRs, no DNS/implicit loopback; PROXY disabled by default with 5s timeout; forwarded disabled by default with 4 KiB/16-element budgets; H3 ignores |
| `tls::TlsServerConfig` / `TlsServerConfigBuilder` / `TlsReloadHandle` / `ClientAuthMode` | experimental | Plan 203/212 neutral `eggnet-tls` identity policy re-exported by EggServe: SNI exact + single-level wildcard + optional default, WebPKI mTLS, atomic reload for new handshakes; CLI/Python stay single-identity; H3 keeps a separate QUIC identity |
| `eggserve_server::interop` (`http-interop`) / `eggserve_server::tower` (`tower`) | experimental (`http-interop` / `tower`) | Optional server-owned ecosystem adapters (Plans 200/276); core's historical module paths re-export them; loss-aware conversions + per-request Tower clones; never in default builds |
| `eggserve.lowlevel.AsyncServer` / `AsyncRequest` / `AsyncResponse` / `Tunnel` (Python) | experimental | Async Python substrate (Plan 204, H1-only): manual asyncio bridge, bounded 16-queues, `max_async_tasks` admission; ASGI fixture in tests only, not a product |
| `Http3Config` | experimental (`http3`) | Bounded QUIC/H3 stream, window, handshake, field-section, idle, retry, and send-buffer policy; deterministic corrective qualification passed under Plan 190, early-error stream scoping added under Plan 192, per-stream producer timeout added under Plan 194 and correctively qualified under Plan 195, while direct H3 remains experimental (`BLOCKED` readiness + Plan 193 retained tier) |
...
| Item | Tier | Notes |
|------|------|-------|
| `ConfinedPath` | stable | Validated path component; `parse()` is the raw-target compatibility adapter and `from_path_component()` is the canonical path-security handoff |
...
| `RequestHead` | stable | Canonical request head: method, target, version, headers, and optional validated authority |
| `RequestHeadError` | stable | Conversion error from Hyper, including invalid or conflicting authority |
...
- Plan 199 supersedes deferred Plan 176 with an experimental generic tunnel handoff (`TunnelRequest`/`TunnelCapability`/`TunnelIo` on `RequestContext`; denial stays ordinary HTTP; WebSocket framing stays downstream). `Service` still returns `Response` only; only `accept` stages a handshake (observed out-of-band by the pipeline; Plan 216, no response-carried token).
...
- Canonical application-facing types, `Service`, and the caller-owned connection API remain Hyper-free for downstream consumers. The explicitly documented `to_hyper_response()` outbound adapter may mention Hyper (inbound Hyper conversion lives in the server transport pipeline); internal runtime implementation use is expected. Tokio channel, PyO3, and platform FFI implementation types remain absent from stable application-facing signatures.
