...
| `TunnelRequest` / `TunnelKind` / `ProtocolName` / `TunnelCapability` / `TunnelIo` / `TunnelError` | experimental, `TunnelError` `#[non_exhaustive]` | Generic tunnel handoff (Plan 199, superseding deferred Plan 176): one-shot `take_tunnel()`, `accept` forges the handshake (`101` H1 / `200` otherwise, runtime owns framing, no raw socket); denial stays ordinary HTTP; WebSocket framing stays downstream |
...
- Plan 199 supersedes deferred Plan 176 with an experimental generic tunnel handoff (`TunnelRequest`/`TunnelCapability`/`TunnelIo` on `RequestContext`; denial stays ordinary HTTP; WebSocket framing stays downstream). `Service` still returns `Response` only; only `accept` forges the handshake token.
