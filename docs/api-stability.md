...
| `tls::TlsServerConfig` / `TlsServerConfigBuilder` / `TlsReloadHandle` / `ClientAuthMode` | experimental | Production TLS identity (Plan 203, Rust-first): SNI exact + single-level wildcard + optional default, WebPKI mTLS, atomic reload for new handshakes; CLI/Python stay single-identity; H3 keeps a separate QUIC identity |
