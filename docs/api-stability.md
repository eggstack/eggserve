...
| `RequestContext` | Experimental | Typed request context / capability container (Plan 197 Track B, extended by Plans 198–199): `connection()` + `lifecycle()` + bounded `interim()` + one-shot `take_tunnel()`; cheap clone never clones the one-shot body; no generic type map, no raw transport handles |
