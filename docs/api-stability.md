...
| `primitives::interop` (`http-interop`) / `server::tower` (`tower`) | experimental (`http-interop` / `tower`) | Optional ecosystem adapters (Plan 200): loss-aware `http`/`http-body` conversions + per-request Tower clones; middleware runs after parsing/validation, before normalization; never in default builds |
