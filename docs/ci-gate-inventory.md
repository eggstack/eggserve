...
| `bash scripts/verify-cargo-packages.sh` | ci.yml / gate/package | linux | — | main | ci-log, package-output | `package.core`, `package.bin` | Both core and bin use same script |
...
| `python3 scripts/release_criteria.py generate-checklist --check` | ci.yml / gate/evidence-aggregate | linux | — | PR, main | ci-log, lint-output | `check-generated` | |
...
|---------|-----------|---------------|
| Pull request | Fast gates: format, clippy, tests, doctests, feature matrix, wire, production-path, corpus, Python source tests, contract consistency, generated files | LOCAL (release-validate.sh) or GITHUB (CI) |
...
- `verify-cargo-packages.sh` is used for both `package.core` and `package.bin` gates. The script validates both crates in a single run.
