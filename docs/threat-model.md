...
4. **Filesystem checks** — when symlink policy denies symlinks, every component of the request path is checked with `symlink_metadata` before it is followed, so intermediate directory symlinks are rejected alongside final symlinks (`PathRejection::SymlinkDenied`). Canonical root escape is rejected with `PathRejection::RootEscapeDenied`. Dotfile policy checks components at both the path-validation and filesystem-resolution layers. The final canonical target is verified to remain inside the configured root. Directory listings also respect symlink policy and hide symlink entries when denied.
