...
- **TE+CL conflict**: Requests containing both `Transfer-Encoding` and `Content-Length` are rejected with 400 before the service is invoked (when both headers survive Hyper's parser normalization). Hyper strips `Content-Length` when `Transfer-Encoding: chunked` is present, so the rejection applies to cases where both headers reach eggserve's validator. Duplicate `Content-Length` fields are always rejected. This prevents HTTP request smuggling attacks where front-end and back-end servers disagree on message boundaries.
