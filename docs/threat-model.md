...
4. **Filesystem checks** — when symlink policy denies symlinks, every component of the request path is checked with `symlink_metadata` before it is followed, so intermediate directory symlinks are rejected alongside final symlinks. Dotfile policy checks components at both the path-validation and filesystem-resolution layers. The final canonical target is verified to remain inside the configured root.
