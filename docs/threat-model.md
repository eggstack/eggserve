...
Static state contains confinement capabilities only, while transport owns the
one file-stream pool and final response headers. A custom Rust or Python service
cannot cause static-root escape.
...
On Unix with safe defaults, descriptor-relative traversal (`statat` + `openat` with `O_NOFOLLOW`) eliminates the TOCTOU window for symlink swaps. On non-Unix or follow-symlinks modes, canonical-path validation and opening remain separated by a mutable-filesystem window, so this attacker is only partially mitigated. In follow-symlinks mode specifically, the post-open re-verification re-canonicalizes the request pathname rather than validating the opened descriptor, so a writer who swaps a symlink away and back between the open and the re-check can obtain a handle to outside-root content while verification passes. This residual double-swap window is an accepted limitation of follow-symlinks mode; it requires local write access to the served tree and never applies to hardened profiles (symlink-denied), which resolve descriptor-relative and do not traverse this code path.

### Windows reparse and namespace attacker

An attacker who can place reparse points (NTFS junctions, symbolic links, mount points) within or adjacent to the serving root on Windows. Under the functional Windows profile, all reparse-point components are denied (tag-independent denial). Parser-level protections reject Windows reserved names, ADS syntax, drive prefixes, and backslash in path components. Handle-relative confinement is implemented including child resolution and handle-relative directory enumeration using `NtQueryDirectoryFile` on the retained directory handle, eliminating the path-based fallback. The adversarial suites cover reparse-point denial, namespace normalization, concurrent mutation races, root identity, file validators, ACL/sharing, resource stability, and installed artifact parity. Two open-descendant root-rename cases are skipped because NTFS rejects that external path operation. Directory listing remains disabled by default. Windows remains functional-only for trusted/local content.

### Resource-exhaustion attacker

An attacker who sends requests designed to consume excessive server resources (memory, CPU, file descriptors, file streams). The server enforces resource limits including: connection count (64 max), one server-wide file-stream pool (32 max), header read timeout (10s), connection total timeout (60s), request target size limits, header count/size limits, and directory listing entry/byte limits (4096 entries max and a 1 MiB response body max; enumeration runs under the 30s default handler timeout; filename lengths are bounded by the filesystem). The built-in static service rejects request bodies by default; custom services may opt into bodies within the runtime ceiling.
...
Promotion to `supported-hardened` requires all gates to pass.
...
- component-by-component handle-relative traversal including child resolution;
...
Windows handle-relative child resolution and directory enumeration are implemented. The adversarial qualification suites are manually exercised. Two open-descendant root-rename cases are skipped due to NTFS path-rename semantics; Windows remains functional-only for trusted/local content.
...
4. **Filesystem checks** — when symlink policy denies symlinks, on Unix, descriptor-relative traversal uses `statat(AT_SYMLINK_NOFOLLOW)` before each `openat(..., O_NOFOLLOW)` to detect symlinks at each path component and to refuse to follow them at open time. Intermediate components are opened with `O_DIRECTORY|O_NOFOLLOW`, final components with `O_RDONLY|O_NOFOLLOW`. On non-Unix or when `--follow-symlinks` is enabled, `symlink_metadata` is checked per component and the final canonical path is verified against the root; this fallback is **weaker** than the descriptor-relative path and is explicitly outside the hardened guarantee. On Windows, handle-relative confinement includes child resolution and directory enumeration using `NtQueryDirectoryFile`; files are opened during resolution — never re-opened later by absolute path. Canonical root escape is rejected with `PathRejection::RootEscapeDenied`. Dotfile policy checks components at both the path-validation and filesystem-resolution layers. Directory listings also respect symlink policy and hide symlink entries when denied.
...
6. **Sanitized logging** — all logged paths and headers are sanitized to prevent log injection.
  7. **Framing enforcement** — duplicate Content-Length and malformed Content-Length are rejected before the service is invoked; a lone Content-Length alongside Transfer-Encoding is discarded by Hyper during parsing with Transfer-Encoding winning (RFC 9112 §6.1), so Hyper remains the single framing authority and front-end/back-end disagreement cannot smuggle bytes past the origin. This prevents request smuggling where front-end and back-end servers disagree on message boundaries.
...
eggserve enforces hardened HTTP/1 framing before body ingestion:
  ...
  - **TE+CL handling**: a lone `Content-Length` alongside `Transfer-Encoding` is discarded by Hyper 1.11 during parsing (`Transfer-Encoding` wins per RFC 9112 §6.1); duplicate `Content-Length` fields are rejected with 400 before the service is invoked. Hyper is the single framing authority, so front-end and origin cannot disagree on message boundaries. This prevents HTTP request smuggling attacks where front-end and back-end servers disagree on message boundaries.
