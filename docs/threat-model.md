...
Parser-level protections reject Windows reserved names, ADS syntax, drive prefixes, and backslash in path components. Plans 075–084 have implemented handle-relative confinement on Windows including child resolution (Plan 084). Plan 085 has implemented handle-relative directory enumeration using `NtQueryDirectoryFile` on the retained directory handle. Reparse-point hardening qualification is pending Plan 086. Directory listing remains disabled by default and outside the hardened production profile. Until qualification completes, Windows is explicitly a trusted/local-use platform — do not use with untrusted mutable public content on Windows. The hardened Windows profile remains candidate/functional-only until production implementation completes.
