...
- **No production profile promotion without evidence** — Production profiles are defined in `release/support-profiles.toml` and require passing all required CI gates (defined in Plan 089 and `release/criteria.toml`) before promotion to hardened status. Gate infrastructure is implemented; external qualification evidence is pending.
