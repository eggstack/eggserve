...
- **No server capability expansion (product-surface freeze)** — Do not add server capabilities outside the existing non-goals without a new explicit product decision. Reject routine feature proposals for: routes/middleware, application handler ecosystems, uploads/forms/multipart, content compression, HTTP/3, WebSockets, reverse proxying, ACME, and virtual hosts. The separately governed, feature-gated native Rust HTTP/2 runtime is limited to Plans 185–186's bounded transport and qualification scope and does not expand the Python compatibility surface. It remains experimental after Plan 186. Correct HTTP/1.1 behavior, security fixes, platform hardening, and bounded compatibility corrections remain in scope.
