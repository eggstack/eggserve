...
- **No Windows hardened profile** — Plans 084–086 implement handle-relative confinement (directory-handle retention, child resolution, directory enumeration via `NtQueryDirectoryFile`) and Plan 129 manually qualifies those suites. Two open-descendant root-rename cases are skipped because NTFS rejects that external path operation, so Windows remains functional-only and trusted/local-content only. See [security-policy.md](security-policy.md) for the full statement.
