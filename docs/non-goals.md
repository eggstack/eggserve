...
- **No Windows hardened profile** — Plans 084–086 implement handle-relative confinement (directory-handle retention, child resolution, directory enumeration via `NtQueryDirectoryFile`) and provide the adversarial filesystem qualification suites. Plan 129 records their manual execution and any remaining test-class caveats. Windows remains functional-only until complete evidence supports a hardened profile. See [security-policy.md](security-policy.md) for the full statement.
