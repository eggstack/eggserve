...
Plan 084 has implemented handle-relative child resolution on Windows. `ResolvedDirectory` retains an owned `OwnedHandle` for handle-relative child resolution, and `RootGuard::resolve_child` uses handle-relative traversal on Windows. This provides stronger confinement than parser-level checks alone. Plan 085 has implemented handle-relative directory enumeration using `NtQueryDirectoryFile`, replacing the path-based fallback. Plan 086 adversarial qualification test scaffold is established (113 tests). Independent safety review and profile promotion decision awaited.
