...
| Invariant | Test coverage |
|-----------|---------------|
| Each chunk uses a fresh bounded initialized-prefix buffer (configurable via `Limits::stream_chunk_size`, default 128 KiB); no unsafe reuse/pool | `canonical.rs` — `file_transport_uses_configured_chunk_size` |
